...
static bool ggml_cuda_should_fuse_mul_mat(const ggml_tensor * ffn_up,
    ...
    if (ggml_is_exl3(ffn_up->src[0]->type) || ggml_is_exl3(ffn_gate->src[0]->type)) {
        if (!is_mul_mat_id || !ggml_is_exl3(ffn_up->src[0]->type) || !ggml_is_exl3(ffn_gate->src[0]->type)) {
            return false;
        }
        if (!ffn_up->src[3] || !ffn_gate->src[3] || !ffn_up->src[4] || !ffn_gate->src[4]) {
            return false;
        }
...
static bool ggml_cuda_should_fuse_mul_mat_vec_q(const ggml_tensor * tensor) {
    ...
    bool use_mul_mat_vec_q = ggml_is_quantized(src0->type) && !ggml_is_exl3(src0->type) && !bad_padding_clear &&
                             src1->type == GGML_TYPE_F32 && dst->type == GGML_TYPE_F32 &&
...
static void ggml_cuda_mul_mat_id(ggml_backend_cuda_context & ctx, ggml_tensor * dst) {
    ...
    if (ggml_is_exl3(src0->type)) {
        ggml_cuda_mul_mat_exl3_id(ctx, dst);
        return;
    }
...
enum ggml_cuda_fwht_fuse_kind {
    GGML_CUDA_FWHT_FUSE_NONE = 0,
    GGML_CUDA_FWHT_FUSE_PRE,
    GGML_CUDA_FWHT_FUSE_POST,
};

struct ggml_cuda_fwht_fuse_match {
    ggml_cuda_fwht_fuse_kind kind = GGML_CUDA_FWHT_FUSE_NONE;
    const ggml_tensor * src       = nullptr;
    const void *        src_data  = nullptr;
    ggml_tensor *       dst       = nullptr;
    const ggml_tensor * scale_in  = nullptr;
    const ggml_tensor * scale_out = nullptr;
    const ggml_tensor * hadamard  = nullptr;
    int                 node_count = 0;
};

static bool ggml_cuda_fwht_tensors_overlap(const ggml_tensor * a, const ggml_tensor * b) {
    if (!a || !b || !a->data || !b->data) {
        return false;
    }
    const int64_t a0 = (int64_t) a->data;
    const int64_t a1 = a0 + (int64_t) ggml_nbytes(a);
    const int64_t b0 = (int64_t) b->data;
    const int64_t b1 = b0 + (int64_t) ggml_nbytes(b);
    return (b0 <= a0 && a0 < b1) || (a0 <= b0 && b0 < a1);
}

static bool ggml_cuda_fwht_is_layout_cast(const ggml_tensor * t) {
    return t && (t->op == GGML_OP_RESHAPE || t->op == GGML_OP_VIEW || t->op == GGML_OP_CONT);
}

static bool ggml_cuda_fwht_is_hadamard(const ggml_tensor * mm) {
    return mm && mm->op == GGML_OP_MUL_MAT &&
           ggml_get_op_params_i32(mm, 1) == GGML_HINT_SRC0_IS_HADAMARD &&
           mm->type == GGML_TYPE_F32 &&
           mm->src[1] && (mm->src[1]->type == GGML_TYPE_F32 || mm->src[1]->type == GGML_TYPE_F16);
}

static bool ggml_cuda_fwht_is_row_vec(const ggml_tensor * v, int64_t ne0) {
    return v && v->type == GGML_TYPE_F32 && ggml_is_contiguous(v) &&
           v->ne[0] == ne0 && v->ne[1] == 1 && v->ne[2] == 1 && v->ne[3] == 1;
}

static bool ggml_cuda_fwht_n_ok(int64_t n) {
    return n == 64 || n == 128 || n == 256 || n == 512;
}

static bool ggml_cuda_fwht_pick_row_scale(
        const ggml_tensor * mul, const ggml_tensor ** act, const ggml_tensor ** scale) {
    ...
    const bool act_ok = (a->type == GGML_TYPE_F32 || a->type == GGML_TYPE_F16);
    const bool scl_ok = (b->type == GGML_TYPE_F32 || b->type == GGML_TYPE_F16);
    if (!act_ok || !scl_ok) {
        return false;
    }
    if (ggml_are_same_shape(a, mul) && ggml_cuda_fwht_is_row_vec(b, mul->ne[0]) && ggml_is_contiguous(a)) {
        *act   = a;
        *scale = b;
        return true;
    }
    if (ggml_are_same_shape(b, mul) && ggml_cuda_fwht_is_row_vec(a, mul->ne[0]) && ggml_is_contiguous(b)) {
        *act   = b;
        *scale = a;
        return true;
    }
...
// Follow RESHAPE/VIEW nodes that re-view `cur`. Returns the last viewed tensor and the next index.
static int ggml_cuda_fwht_follow_casts(
        const ggml_cgraph * cgraph, int i, const ggml_tensor * cur, const ggml_tensor ** viewed) {
    *viewed = cur;
    while (i < cgraph->n_nodes && ggml_cuda_fwht_is_layout_cast(cgraph->nodes[i]) &&
           cgraph->nodes[i]->src[0] == *viewed) {
...
// MUL(x, v[ne0]) then FWHT, or FWHT then MUL(x, v[ne0]). RESHAPE/VIEW between them may be present or elided.
static bool ggml_cuda_match_fwht_scale(const ggml_cgraph * cgraph, int node_idx, ggml_cuda_fwht_fuse_match & match) {
    if (node_idx + 1 >= cgraph->n_nodes) {
        return false;
    }
    ...
    const int           after  = ggml_cuda_fwht_follow_casts(cgraph, node_idx + 1, n0, &viewed);
    if (after >= cgraph->n_nodes) {
        return false;
    }
    ggml_tensor * n_last = cgraph->nodes[after];

    if (n0->op == GGML_OP_MUL && ggml_cuda_fwht_is_hadamard(n_last)) {
        if (!ggml_cuda_fwht_pick_row_scale(n0, &act, &scale)) {
            return false;
        }
        const ggml_tensor * fwht_src = n_last->src[1];
        if (fwht_src != viewed) {
            if (!ggml_cuda_fwht_is_layout_cast(fwht_src) || fwht_src->src[0] != n0) {
                return false;
            }
        }
        if (!ggml_cuda_fwht_n_ok(fwht_src->ne[0]) || ggml_nelements(act) != ggml_nelements(fwht_src)) {
            return false;
        }
        if ((n0->flags & GGML_TENSOR_FLAG_OUTPUT) || ggml_node_get_use_count(cgraph, node_idx) != 1) {
            return false;
        }
        for (int k = node_idx + 1; k < after; ++k) {
            if ((cgraph->nodes[k]->flags & GGML_TENSOR_FLAG_OUTPUT) || ggml_node_get_use_count(cgraph, k) != 1) {
                return false;
            }
        }
        match.kind       = GGML_CUDA_FWHT_FUSE_PRE;
        match.src        = fwht_src;
        match.src_data   = act->data;
        match.dst        = n_last;
        match.scale_in   = scale;
        match.scale_out  = nullptr;
        match.hadamard   = n_last->src[0];
        match.node_count = after - node_idx + 1;
    } else if (ggml_cuda_fwht_is_hadamard(n0) && n_last->op == GGML_OP_MUL) {
        if (!ggml_cuda_fwht_pick_row_scale(n_last, &act, &scale)) {
            return false;
        }
        if (act != viewed) {
            if (!ggml_cuda_fwht_is_layout_cast(act) || act->src[0] != n0) {
                return false;
            }
        }
        if (!ggml_cuda_fwht_n_ok(n0->src[1]->ne[0])) {
            return false;
        }
        if ((n0->flags & GGML_TENSOR_FLAG_OUTPUT) || ggml_node_get_use_count(cgraph, node_idx) != 1) {
            return false;
        ...
        match.kind       = GGML_CUDA_FWHT_FUSE_POST;
        match.src        = n0->src[1];
        match.src_data   = n0->src[1]->data;
        match.dst        = n_last;
        match.scale_in   = nullptr;
        match.scale_out  = scale;
        match.hadamard   = n0->src[0];
        match.node_count = after - node_idx + 1;
    } else {
        return false;
    }

    if (match.node_count < 2) {
        return false;
    }

    // dest may alias the FWHT source (row-wise in-place is safe). do not alias scales or H.
    if (ggml_cuda_fwht_tensors_overlap(match.dst, match.scale_in) ||
        ggml_cuda_fwht_tensors_overlap(match.dst, match.scale_out) ||
        ggml_cuda_fwht_tensors_overlap(match.dst, match.hadamard)) {
        return false;
    }
...
struct ggml_cuda_exl3_in_fuse_match {
...
static bool ggml_cuda_exl3_cast_uses_ok(const ggml_cgraph * cgraph, int begin, int end) {
...
static bool ggml_cuda_exl3_from_had(const ggml_tensor * t, const ggml_tensor * had) {
    for (int hops = 0; t && hops < 6; ++hops) {
        if (t == had) {
            return true;
        }
        if (!ggml_cuda_fwht_is_layout_cast(t)) {
            return false;
        }
        t = t->src[0];
    }
...
// T<=8: input-only or full chain. T>8: full chain only, if recon_had is enabled.
#ifndef EXL3_RECON_HAD_MIN_M
#define EXL3_RECON_HAD_MIN_M 192
#endif
static bool ggml_cuda_exl3_fuse_ok_for_m(int64_t M, bool full) {
    ...
    return M >= EXL3_RECON_HAD_MIN_M;
}

// MUL(x, suh) + [RESHAPE|VIEW|CONT]* + MUL_MAT(H128) + casts + MUL_MAT(EXL3)
// optional: + casts + MUL_MAT(H128) + casts + MUL(res, svh)
static bool ggml_cuda_match_exl3_in_fuse(const ggml_cgraph * cgraph, int node_idx,
                                         ggml_cuda_exl3_in_fuse_match & match) {
    ...
    const ggml_tensor * viewed = n0;
    const int after_mul = ggml_cuda_fwht_follow_casts(cgraph, node_idx + 1, n0, &viewed);
    if (after_mul >= cgraph->n_nodes || !ggml_cuda_exl3_cast_uses_ok(cgraph, node_idx + 1, after_mul)) {
        return false;
    }

    ggml_tensor * had = cgraph->nodes[after_mul];
    if (!ggml_cuda_fwht_is_hadamard(had) || had->src[1]->ne[0] != 128) {
        return false;
    }
    const ggml_tensor * fwht_src = had->src[1];
    if (fwht_src != viewed) {
        if (!ggml_cuda_fwht_is_layout_cast(fwht_src) || fwht_src->src[0] != n0) {
            return false;
        }
    }
    if (ggml_nelements(act) != ggml_nelements(fwht_src)) {
        return false;
    }
    ...
    int after_had = ggml_cuda_fwht_follow_casts(cgraph, after_mul + 1, had, &viewed2);
    if (after_had >= cgraph->n_nodes || !ggml_cuda_exl3_cast_uses_ok(cgraph, after_mul + 1, after_had)) {
        return false;
    }
    ...
    if (mm->op != GGML_OP_MUL_MAT || !mm->src[0] || !ggml_is_exl3(mm->src[0]->type) ||
        ...
        for (int j = after_had; j < scan_end; ++j) {
            ...
            if (!ggml_cuda_exl3_from_had(cand->src[1], had)) {
                continue;
            }
            bool chain = true;
            for (int k = after_had; k < j; ++k) {
                if (!ggml_cuda_fwht_is_layout_cast(cgraph->nodes[k]) ||
                        !ggml_cuda_exl3_from_had(cgraph->nodes[k], had) ||
                        (cgraph->nodes[k]->flags & GGML_TENSOR_FLAG_OUTPUT) ||
                        ggml_node_get_use_count(cgraph, k) != 1) {
                    chain = false;
                    break;
                }
            }
    ...
    if (mm->src[1] != viewed2) {
        if (!ggml_cuda_exl3_from_had(mm->src[1], had)) {
            return false;
        }
    }
    if (ggml_cuda_fwht_tensors_overlap(mm, scale) || ggml_cuda_fwht_tensors_overlap(mm, mm->src[0])) {
        return false;
    }
    ...
    match.w          = mm->src[0];
    match.dst        = mm;
    match.node_count = after_had - node_idx + 1;
    match.full       = false;
    if (match.node_count < 3) {
        return false;
    }

    const int64_t fuse_m = ggml_nrows(act);
    const ggml_tensor * viewed3 = mm;
    const int after_mm = ggml_cuda_fwht_follow_casts(cgraph, after_had + 1, mm, &viewed3);
    if (after_mm >= cgraph->n_nodes || !ggml_cuda_exl3_cast_uses_ok(cgraph, after_had + 1, after_mm)) {
        return ggml_cuda_exl3_fuse_ok_for_m(fuse_m, false);
    }
    ggml_tensor * had2 = cgraph->nodes[after_mm];
    if (!ggml_cuda_fwht_is_hadamard(had2) || had2->src[1]->ne[0] != 128) {
        return ggml_cuda_exl3_fuse_ok_for_m(fuse_m, false);
    }
    if (had2->src[1] != viewed3 && !ggml_cuda_exl3_from_had(had2->src[1], mm)) {
        return ggml_cuda_exl3_fuse_ok_for_m(fuse_m, false);
    }
    if ((had2->flags & GGML_TENSOR_FLAG_OUTPUT) || ggml_node_get_use_count(cgraph, after_mm) != 1) {
        return ggml_cuda_exl3_fuse_ok_for_m(fuse_m, false);
    }

    const ggml_tensor * viewed4 = had2;
    const int after_had2 = ggml_cuda_fwht_follow_casts(cgraph, after_mm + 1, had2, &viewed4);
    if (after_had2 >= cgraph->n_nodes || !ggml_cuda_exl3_cast_uses_ok(cgraph, after_mm + 1, after_had2)) {
        return ggml_cuda_exl3_fuse_ok_for_m(fuse_m, false);
    }
    ggml_tensor * mul_svh = cgraph->nodes[after_had2];
    const ggml_tensor * act2 = nullptr;
    const ggml_tensor * svh  = nullptr;
    if (!ggml_cuda_fwht_pick_row_scale(mul_svh, &act2, &svh)) {
        return ggml_cuda_exl3_fuse_ok_for_m(fuse_m, false);
    }
    if (act2 != viewed4 && !ggml_cuda_exl3_from_had(act2, had2)) {
        return ggml_cuda_exl3_fuse_ok_for_m(fuse_m, false);
    }
    if (svh->ne[0] != mm->ne[0] || ggml_nelements(mul_svh) != ggml_nelements(mm)) {
        return ggml_cuda_exl3_fuse_ok_for_m(fuse_m, false);
    }
    if (ggml_cuda_fwht_tensors_overlap(mul_svh, scale) || ggml_cuda_fwht_tensors_overlap(mul_svh, svh) ||
        ggml_cuda_fwht_tensors_overlap(mul_svh, mm->src[0])) {
        return ggml_cuda_exl3_fuse_ok_for_m(fuse_m, false);
    }

    match.svh        = svh;
    match.dst        = mul_svh;
    match.node_count = after_had2 - node_idx + 1;
    match.full       = true;
    return ggml_cuda_exl3_fuse_ok_for_m(fuse_m, true);
}

static int64_t g_fwht_fuse_pre  = 0;
static int64_t g_fwht_fuse_post = 0;
static int64_t g_exl3_in_fuse   = 0;
static int64_t g_exl3_full_fuse = 0;

static bool ggml_cuda_fwht_fuse_debug_on() {
    static const bool on = [] {
        const char * e = getenv("GGML_CUDA_FUSION_DEBUG");
        return e != nullptr && std::atoi(e) != 0;
    }();
    return on;
}

static void ggml_cuda_fwht_fuse_debug(const ggml_cuda_fwht_fuse_match & match) {
    if (!ggml_cuda_fwht_fuse_debug_on()) {
        return;
    }
    if (match.kind == GGML_CUDA_FWHT_FUSE_PRE) {
        ++g_fwht_fuse_pre;
    } else {
        ++g_fwht_fuse_post;
    }
...
static void ggml_cuda_fwht_fuse_debug_graph(int n_nodes) {
    if (!ggml_cuda_fwht_fuse_debug_on()) {
        return;
    }
    static int64_t last_pre  = 0;
    static int64_t last_post = 0;
    static int64_t last_exl3 = 0;
    static int64_t last_full = 0;
    const int64_t  dpre      = g_fwht_fuse_pre - last_pre;
    const int64_t  dpost     = g_fwht_fuse_post - last_post;
    const int64_t  dexl3     = g_exl3_in_fuse - last_exl3;
    const int64_t  dfull     = g_exl3_full_fuse - last_full;
    if (dpre == 0 && dpost == 0 && dexl3 == 0 && dfull == 0) {
        return;
    }
    fprintf(stderr, "fwht scale fusion graph: pre=%ld post=%ld exl3_in=%ld exl3_full=%ld fused_nodes=%ld n_nodes=%d (cum pre=%ld post=%ld exl3=%ld full=%ld)\n",
            (long) dpre, (long) dpost, (long) dexl3, (long) dfull,
            (long) ((dpre + dpost) * 3 + (dexl3 - dfull) * 5 + dfull * 9), n_nodes,
            (long) g_fwht_fuse_pre, (long) g_fwht_fuse_post, (long) g_exl3_in_fuse, (long) g_exl3_full_fuse);
    last_pre  = g_fwht_fuse_pre;
    last_post = g_fwht_fuse_post;
    last_exl3 = g_exl3_in_fuse;
    last_full = g_exl3_full_fuse;
}
...
static bool ggml_cuda_can_fuse(const struct ggml_cgraph *                cgraph,
    ...
    std::initializer_list<enum ggml_op> mul_mat_id_glu_down_ops = { GGML_OP_MUL_MAT_ID, GGML_OP_MUL_MAT_ID, GGML_OP_GLU, GGML_OP_MUL_MAT_ID };
    std::initializer_list<enum ggml_op> mul_mat_id_glu_ops = { GGML_OP_MUL_MAT_ID, GGML_OP_MUL_MAT_ID, GGML_OP_GLU };
    std::initializer_list<enum ggml_op> mul_mat_glu_ops    = { GGML_OP_MUL_MAT,    GGML_OP_MUL_MAT,    GGML_OP_GLU };

    if (is_equal(mul_mat_id_glu_down_ops, ops) &&
        ggml_can_fuse_subgraph(cgraph, node_idx, ops, { node_idx + 3 })) {
        const ggml_tensor * ffn_gate = cgraph->nodes[node_idx];
        const ggml_tensor * ffn_up   = cgraph->nodes[node_idx + 1];
        const ggml_tensor * glu      = cgraph->nodes[node_idx + 2];
        const ggml_tensor * ffn_down = cgraph->nodes[node_idx + 3];
        if (ggml_cuda_should_fuse_mul_mat(ffn_up, ffn_gate, glu) &&
                ffn_down->src[1] == glu && ffn_down->src[2] == ffn_up->src[2] &&
                ggml_is_exl3(ffn_down->src[0]->type) && ffn_down->src[3] && ffn_down->src[4]) {
    ...
    std::initializer_list<enum ggml_op> mul_reshape_fwht = { GGML_OP_MUL, GGML_OP_RESHAPE, GGML_OP_MUL_MAT };
    std::initializer_list<enum ggml_op> mul_view_fwht    = { GGML_OP_MUL, GGML_OP_VIEW,    GGML_OP_MUL_MAT };
    std::initializer_list<enum ggml_op> mul_fwht         = { GGML_OP_MUL, GGML_OP_MUL_MAT };
    std::initializer_list<enum ggml_op> fwht_reshape_mul = { GGML_OP_MUL_MAT, GGML_OP_RESHAPE, GGML_OP_MUL };
    std::initializer_list<enum ggml_op> fwht_view_mul    = { GGML_OP_MUL_MAT, GGML_OP_VIEW,    GGML_OP_MUL };
    std::initializer_list<enum ggml_op> fwht_mul         = { GGML_OP_MUL_MAT, GGML_OP_MUL };
    std::initializer_list<enum ggml_op> exl3_in_ops      = { GGML_OP_MUL, GGML_OP_RESHAPE, GGML_OP_MUL_MAT, GGML_OP_RESHAPE, GGML_OP_MUL_MAT };
    std::initializer_list<enum ggml_op> exl3_in_view_ops = { GGML_OP_MUL, GGML_OP_VIEW,    GGML_OP_MUL_MAT, GGML_OP_VIEW,    GGML_OP_MUL_MAT };
    std::initializer_list<enum ggml_op> exl3_full_ops = {
        GGML_OP_MUL, GGML_OP_RESHAPE, GGML_OP_MUL_MAT, GGML_OP_RESHAPE, GGML_OP_MUL_MAT,
        GGML_OP_RESHAPE, GGML_OP_MUL_MAT, GGML_OP_RESHAPE, GGML_OP_MUL
    };
    std::initializer_list<enum ggml_op> exl3_full_view_ops = {
        GGML_OP_MUL, GGML_OP_VIEW, GGML_OP_MUL_MAT, GGML_OP_VIEW, GGML_OP_MUL_MAT,
        GGML_OP_VIEW, GGML_OP_MUL_MAT, GGML_OP_VIEW, GGML_OP_MUL
    };

    if (is_equal(exl3_in_ops, ops) || is_equal(exl3_in_view_ops, ops) ||
            is_equal(exl3_full_ops, ops) || is_equal(exl3_full_view_ops, ops)) {
        ggml_cuda_exl3_in_fuse_match em;
        return ggml_cuda_match_exl3_in_fuse(cgraph, node_idx, em);
    }

    if (is_equal(mul_reshape_fwht, ops) || is_equal(mul_view_fwht, ops) || is_equal(mul_fwht, ops) ||
        is_equal(fwht_reshape_mul, ops) || is_equal(fwht_view_mul, ops) || is_equal(fwht_mul, ops)) {
        ggml_cuda_fwht_fuse_match match;
        return ggml_cuda_match_fwht_scale(cgraph, node_idx, match);
    }
...
static int ggml_cuda_try_fuse(ggml_backend_cuda_context * cuda_ctx, ggml_cgraph * cgraph, int i) {
    ...
    if (node->op == GGML_OP_MUL) {
        ggml_cuda_exl3_in_fuse_match em;
        if (ggml_cuda_match_exl3_in_fuse(cgraph, i, em)) {
            ggml_cuda_mul_mat_exl3(*cuda_ctx, em.w, nullptr, em.dst, em.x, em.suh, em.svh);
            if (ggml_cuda_fwht_fuse_debug_on()) {
                ++g_exl3_in_fuse;
                if (em.full) {
                    ++g_exl3_full_fuse;
            ...
            GGML_LOG_INFO("%s: fused exl3 %s suh+fwht+gemv for %s (skip %d)\n", __func__,
                          em.full ? "full" : "input", node->name, em.node_count - 1);
#endif
            return em.node_count - 1;
        }
        if (ggml_cuda_fwht_fuse_debug_on()) {
            const ggml_tensor * act = nullptr;
            const ggml_tensor * scale = nullptr;
            bool had_soon = false;
            const int look = i + 6 < cgraph->n_nodes ? i + 6 : cgraph->n_nodes;
            for (int k = i + 1; k < look; ++k) {
                if (ggml_cuda_fwht_is_hadamard(cgraph->nodes[k])) {
                    had_soon = true;
                    break;
                }
            }
            if (had_soon && ggml_cuda_fwht_pick_row_scale(node, &act, &scale) && act &&
                    act->ne[0] % 128 == 0 && ggml_nrows(act) <= EXL3_GEMV_MAX_M) {
                static int nmiss = 0;
                if (nmiss < 16) {
                    fprintf(stderr, "exl3_in miss #%d node=%d name=%s act=[%ld,%ld,%ld,%ld] contig=%d contig1=%d nb1=%ld rowb=%ld uses:",
                            nmiss, i, node->name,
                            (long) act->ne[0], (long) act->ne[1], (long) act->ne[2], (long) act->ne[3],
                            (int) ggml_is_contiguous(act), (int) ggml_is_contiguous_1(act),
                            (long) act->nb[1], (long) (act->ne[0] * act->nb[0]));
                    const int end = i + 9 < cgraph->n_nodes ? i + 9 : cgraph->n_nodes;
                    for (int k = i; k < end; ++k) {
                        const ggml_tensor * t = cgraph->nodes[k];
                        const char * extra = "";
                        if (t->op == GGML_OP_MUL_MAT && t->src[0] && ggml_is_exl3(t->src[0]->type)) {
                            extra = "/exl3";
                        } else if (ggml_cuda_fwht_is_hadamard(t)) {
                            extra = "/had";
                        }
                        fprintf(stderr, " %s%s(u=%d)", ggml_op_name(t->op), extra, (int) ggml_node_get_use_count(cgraph, k));
                    }
                    fprintf(stderr, "\n");
                    ++nmiss;
                }
    ...
    if (node->op == GGML_OP_MUL || node->op == GGML_OP_MUL_MAT) {
        ggml_cuda_fwht_fuse_match match;
        if (ggml_cuda_match_fwht_scale(cgraph, i, match) &&
            ggml_cuda_op_fwht(*cuda_ctx, match.src, match.dst, match.scale_in, match.scale_out, match.src_data)) {
            ggml_cuda_fwht_fuse_debug(match);
#ifdef GGML_CUDA_DEBUG
            GGML_LOG_INFO("%s: fused fwht %s-scale for %s (skip %d)\n", __func__,
                          match.kind == GGML_CUDA_FWHT_FUSE_PRE ? "pre" : "post",
                          node->name, match.node_count - 1);
#endif
    ...
    for (ggml_op op : { GGML_OP_MUL_MAT, GGML_OP_MUL_MAT_ID }) {
        ...
        if (ggml_cuda_can_fuse(cgraph, i, { op, bias_op, op, bias_op, GGML_OP_GLU }, {})) {
            ...
            if (ids && ggml_is_exl3(src0->type) && ggml_is_exl3(gate->src[0]->type) &&
                    ggml_get_glu_op(glu) == GGML_GLU_OP_SWIGLU &&
                    gate->src[3] && gate->src[4] && up->src[3] && up->src[4]) {
                ggml_tensor * down = nullptr;
                if (i + 3 < cgraph->n_nodes && cgraph->nodes[i + 3]->op == GGML_OP_MUL_MAT_ID) {
                    ggml_tensor * d = cgraph->nodes[i + 3];
                    if (d->src[1] == glu && d->src[2] == ids && ggml_is_exl3(d->src[0]->type) &&
                            d->src[3] && d->src[4]) {
                        const ggml_op ops4[] = { GGML_OP_MUL_MAT_ID, GGML_OP_MUL_MAT_ID, GGML_OP_GLU, GGML_OP_MUL_MAT_ID };
                        const int out4[] = { i + 3 };
                        if (ggml_can_fuse_subgraph(cgraph, i, 4, ops4, out4, 1) &&
                                ggml_cuda_check_fusion_memory_ranges(cgraph, i, 4, out4, 1)) {
                            down = d;
                        }
                    }
                }
                ggml_cuda_mul_mat_exl3_id_glu(*cuda_ctx, gate, up, glu, down);
                fused_mul_mat_vec = true;
                fused_node_count  = down ? 4 : 3;
                break;
            }
        }
    }
...
static void ggml_cuda_graph_evaluate_and_capture(ggml_backend_cuda_context * cuda_ctx, ggml_cgraph * cgraph, const bool use_cuda_graph, const bool cuda_graph_update_required, const void * graph_key) {
    ...
    while (!graph_evaluated_or_captured) {
        ...
        if (!use_cuda_graph || cuda_graph_update_required) {
            ...
            ggml_cuda_fwht_fuse_debug_graph(cgraph->n_nodes);
        }
