...
static void ggml_cpy_f32_pack_rows(const float * src, float * dst, int64_t ne0, int64_t ne1, int64_t ne2, int64_t ne3,
                                   int64_t s0, int64_t s1, int64_t s2, int64_t s3, queue_ptr stream) {
    const int64_t n_rows = ne1 * ne2 * ne3;
    const int     wg     = 256;
    const int64_t nblk   = (n_rows + wg - 1) / wg;
    stream->parallel_for(sycl::nd_range<1>(sycl::range<1>((size_t) nblk * wg), sycl::range<1>(wg)),
                         [=](sycl::nd_item<1> item) {
                             ...
                             const float * sr  = src + i1 * s1 + i2 * s2 + i3 * s3;
...
#endif
    } else if (src0->type == GGML_TYPE_F32 && src1->type == GGML_TYPE_F32 && ggml_is_contiguous(src1) &&
               src0->nb[0] == sizeof(float) && src0->ne[0] > 0 && src0->ne[0] <= 8 &&
               ggml_sycl_get_env("GGML_SYCL_CPY_PACK", 1)) {
        ggml_cpy_f32_pack_rows((const float *) src0_ddc, (float *) src1_ddc, src0->ne[0], src0->ne[1], src0->ne[2],
                               src0->ne[3], 1, (int64_t) (src0->nb[1] / sizeof(float)),
                               (int64_t) (src0->nb[2] / sizeof(float)), (int64_t) (src0->nb[3] / sizeof(float)),
                               main_stream);
