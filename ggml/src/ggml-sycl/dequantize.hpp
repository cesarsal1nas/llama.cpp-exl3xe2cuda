...
template <typename dst_t>
static void dequantize_block_iq4_xs_reorder(const void * __restrict__ vx, dst_t * __restrict__ yy,
                                            const sycl::nd_item<3> & item_ct1, int64_t nb, int64_t i0 = 0) {
    const int64_t i   = item_ct1.get_group(2) + i0;
    const int64_t tid = item_ct1.get_local_id(2);
    const int64_t il  = tid / 8;
    const int64_t ib  = tid % 8;

    const uint8_t * base     = static_cast<const uint8_t *>(vx);
    const uint8_t * qs       = base + i * (QK_K / 2);
    const uint8_t * scales_l = base + nb * (QK_K / 2) + i * 4;
    const uint8_t * packed   = base + nb * (QK_K / 2) + nb * 4 + i * 4;
    const uint16_t  scales_h = *reinterpret_cast<const uint16_t *>(packed);
    const ggml_half d0       = *reinterpret_cast<const ggml_half *>(packed + 2);

    dst_t *         y  = yy + item_ct1.get_group(2) * QK_K + 32 * ib + 4 * il;
    const uint8_t * q4 = qs + 16 * ib + 4 * il;
    const float     d =
        float(d0) * float((((scales_l[ib / 2] >> 4 * (ib % 2)) & 0xf) | (((scales_h >> 2 * ib) & 3) << 4)) - 32);
#pragma unroll
    for (int j = 0; j < 4; ++j) {
        y[j + 0]  = d * kvalues_iq4nl[q4[j] & 0xf];
        y[j + 16] = d * kvalues_iq4nl[q4[j] >> 4];
