...
template <> struct reorder_vec_dot_q_sycl<GGML_TYPE_IQ4_XS> {
    ...
    using iq4_xs_block  = ggml_sycl_reordered::block_q_t<GGML_TYPE_IQ4_XS>;
    using iq4_xs_traits = typename iq4_xs_block::traits;
    ...
    __dpct_inline__ float operator()(const void * __restrict__ vbq, const std::pair<int, int> ibx_offset,
        ...
        const sycl::vec<uint32_t, 4> q4 =
            *reinterpret_cast<const sycl::vec<uint32_t, 4> *>(base + ibx_offset.first + 16 * iqs);
        const uint32_t  scales_l32 = *reinterpret_cast<const uint32_t *>(base + d_offset.first);
        const uint32_t  hd         = *reinterpret_cast<const uint32_t *>(base + d_offset.second);
        const uint16_t  scales_h   = static_cast<uint16_t>(hd);
        const ggml_half d0         = sycl::bit_cast<ggml_half>(static_cast<uint16_t>(hd >> 16));

        const int8_t ls = ((scales_l32 >> (4 * iqs)) & 0xf) | (((scales_h >> (2 * iqs)) & 3) << 4);
        const int8_t * q8_base = q8_1_quant_ptr + iqs * QK8_1;
        const sycl::vec<int, 4> q8_lo = *reinterpret_cast<const sycl::vec<int, 4> *>(q8_base);
        const sycl::vec<int, 4> q8_hi = *reinterpret_cast<const sycl::vec<int, 4> *>(q8_base + 16);
        const float  d  = float(d0) * float(ls - 32) * float(q8_1_ds[iqs][0]);

        const sycl::vec<uint32_t, 4> lut =
            *reinterpret_cast<const sycl::vec<uint32_t, 4> *>(kvalues_iq4nl);
        const uint8_t * values = reinterpret_cast<const uint8_t *>(&lut);
        int sumi1 = 0, sumi2 = 0;
        int v1, v2;
#pragma unroll
        for (int j = 0; j < 4; ++j) {
            get_int_from_table_16(q4[j], values, v1, v2);
            sumi1 = dpct::dp4a(v1, q8_lo[j], sumi1);
            sumi2 = dpct::dp4a(v2, q8_hi[j], sumi2);
        }
        return d * (sumi1 + sumi2);
    }
};
