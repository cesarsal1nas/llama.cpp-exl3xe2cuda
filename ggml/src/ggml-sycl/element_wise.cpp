...
void ggml_sycl_op_add_softplus_mul(ggml_backend_sycl_context & ctx, ggml_tensor * add, ggml_tensor * softplus,
                                   ggml_tensor * mul) {
    scope_op_debug_print scope_dbg_print(__func__, mul, /*num_src=*/2);

    const ggml_tensor * a = add->src[0];
    const ggml_tensor * b = add->src[1];
    const ggml_tensor * w = (mul->src[0] == softplus) ? mul->src[1] : mul->src[0];

    GGML_ASSERT(a && b && w);
    GGML_ASSERT(a->type == GGML_TYPE_F32 && b->type == GGML_TYPE_F32 && w->type == GGML_TYPE_F32);
    GGML_ASSERT(mul->type == GGML_TYPE_F32);
    GGML_ASSERT(ggml_is_contiguous_1(a) && ggml_is_contiguous(b) && ggml_is_contiguous(w) && ggml_is_contiguous(mul));

    const int64_t k  = ggml_nelements(mul);
    const int64_t nb = ggml_nelements(b);
    const int64_t nw = ggml_nelements(w);
    GGML_ASSERT(k > 0 && nb > 0 && nw > 0 && (k % nb) == 0 && (k % nw) == 0);
    ...
    const bool    a_pack = ggml_is_contiguous(a);
    ...
    main_stream->parallel_for(range, [=](sycl::nd_item<1> item_ct1) [[sycl::reqd_sub_group_size(WARP_SIZE)]] {
        SYCL_GLOBAL_ID_LOOP(k, item_ct1) {
            const int64_t ai = a_pack ? i : ((i / n0) * a_s1 + (i % n0));
            d_d[i] = op_softplus(a_d[ai] + b_d[i % nb]) * w_d[i % nw];
        }
    });
}
