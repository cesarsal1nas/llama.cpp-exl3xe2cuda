        ...
        switch (src0->type) {
            ...
            case GGML_TYPE_Q4_K:
                if ((ggml_tensor_extra_gpu *) dst->src[0]->extra &&
                    ((ggml_tensor_extra_gpu *) dst->src[0]->extra)->optimized_feature.reorder) {
                    if (ctx.q4k_y_slots) {
                        GGML_ASSERT(src1_ncols <= GGML_SYCL_IQ4_ESIMD_MAX_NCOLS);
                        const int stride_col_y_bytes = src1_padded_col_size * q8_1_ts / q8_1_bs;
                        const int stride_col_dst     = dst->ne[0];
                        const bool ok = ggml_sycl_mul_mat_vec_q4_K_q8_1_esimd(src0_dd_i, src1_ddq_i, dst_dd_i, ne00,
                                                                              row_diff, (int) src1_ncols,
                                                                              stride_col_y_bytes, stride_col_dst, stream);
                        GGML_ASSERT(ok && "Q4_K ESIMD kernel declined a slot-ordered y");
                        return;
                    }
                    if (i == 0 && src1_ncols > 1 && src1_ncols <= 8) {
                        const int stride_col_y_bytes = src1_padded_col_size * q8_1_ts / q8_1_bs;
            ...
            case GGML_TYPE_Q5_K:
                if ((ggml_tensor_extra_gpu *) dst->src[0]->extra &&
                    ...
                    if (ctx.q5k_y_slots) {
                        ...
                        const int stride_col_y_bytes = src1_padded_col_size * q8_1_ts / q8_1_bs;
                        const int stride_col_dst     = dst->ne[0];
                        const bool ok = ggml_sycl_mul_mat_vec_q5_K_q8_1_esimd(src0_dd_i, src1_ddq_i, dst_dd_i, ne00,
                                                                              row_diff, (int) src1_ncols,
                                                                              stride_col_y_bytes, stride_col_dst, stream);
                        GGML_ASSERT(ok && "Q5_K ESIMD kernel declined a slot-ordered y");
                        return;
                    }
                    if (i == 0 && src1_ncols > 1 && src1_ncols <= 8) {
                        const int stride_col_y_bytes = src1_padded_col_size * q8_1_ts / q8_1_bs;
            ...
            case GGML_TYPE_IQ4_XS:
                if ((ggml_tensor_extra_gpu *) dst->src[0]->extra &&
                    ...
                    const int stride_col_y_bytes = src1_padded_col_size * q8_1_ts / q8_1_bs;
                    const int stride_col_dst     = dst->ne[0];
                    const bool ok = ggml_sycl_mul_mat_vec_iq4_xs_q8_1_esimd(src0_dd_i, src1_ddq_i, dst_dd_i, ne00,
                                                                            row_diff, (int) src1_ncols,
                                                                            stride_col_y_bytes, stride_col_dst, stream);
                    GGML_ASSERT(ok && "IQ4_XS ESIMD kernel declined a pair-ordered y");
                    return;
                }
                if (i == 0 && src1_ncols > 1 && src1_ncols <= 8) {
                    const int stride_col_y   = src1_padded_col_size / QK8_1;
...
bool ggml_sycl_mul_mat_vec_q_id(
    ...
    dpct::queue_ptr    stream,
    int                n_tokens,
    size_t             ids_tok_stride,
    size_t             dst_tok_stride,
    size_t             src1_tok_stride) {
    if (n_tokens > 1) {
        bool ok = true;
        for (int tok = 0; tok < n_tokens && ok; ++tok) {
            const int32_t * ids_t = (const int32_t *) ((const char *) ids_dev + (size_t) tok * ids_tok_stride);
            const char *    y_t   = (const char *) vy + (size_t) tok * src1_tok_stride;
            float *         d_t   = (float *) ((char *) dst_base + (size_t) tok * dst_tok_stride);
            ok = ggml_sycl_mul_mat_vec_q_id(
                src0_type, vx_base, y_t, ids_t, d_t, ncols, nrows, n_experts_used,
...
static void mul_mat_vec_q_moe_reorder(
    const void * __restrict__ vx_base, const void * __restrict__ vy_base,
    float * __restrict__ dst_base, const int32_t * __restrict__ ids_base,
    const int ncols, const int nrows,
    const size_t expert_weight_stride, const size_t dst_row_stride,
    const size_t src1_row_stride,
    const size_t ids_tok_stride, const size_t dst_tok_stride, const size_t src1_tok_stride,
    const sycl::nd_item<3> & item_ct1) {
    using block_type   = ggml_sycl_reordered::block_q_t<reorder_vec_dot_q_sycl::gtype>;
    using block_traits = typename block_type::traits;

    const int tok        = item_ct1.get_group(0);
    const int expert_idx = item_ct1.get_group(1);
    const int32_t * ids_dev = (const int32_t *) ((const char *) ids_base + (size_t) tok * ids_tok_stride);
    const int i02        = ids_dev[expert_idx];

    const char * vx  = (const char *) vx_base + (size_t) i02 * expert_weight_stride;
    const char * vy  = (const char *) vy_base + (size_t) tok * src1_tok_stride
                     + (size_t) expert_idx * src1_row_stride;
    float *      dst = (float *) ((char *) dst_base + (size_t) tok * dst_tok_stride
...
static void launch_mul_mat_vec_q_moe_reorder(
    ...
    const size_t expert_weight_stride, const size_t dst_row_stride,
    const size_t src1_row_stride,
    int n_tokens, size_t ids_tok_stride, size_t dst_tok_stride, size_t src1_tok_stride,
    dpct::queue_ptr stream) {
    const int            block_num_y = (nrows + GGML_SYCL_MMV_Y - 1) / GGML_SYCL_MMV_Y;
    const unsigned       ntok        = (unsigned) (n_tokens > 0 ? n_tokens : 1);
    const sycl::range<3> block_nums(ntok, (unsigned) n_experts_used, (unsigned) block_num_y);
    const sycl::range<3> block_dims(1, GGML_SYCL_MMV_Y, WARP_SIZE);
    stream->submit([&](sycl::handler & cgh) {
        cgh.parallel_for(
            sycl::nd_range<3>(block_nums * block_dims, block_dims),
            [=](sycl::nd_item<3> item) [[sycl::reqd_sub_group_size(WARP_SIZE)]] {
                mul_mat_vec_q_moe_reorder<reorder_vec_dot_q_sycl>(
                    ...
                    ids_tok_stride, dst_tok_stride, src1_tok_stride, item);
            });
    });
}
...
bool ggml_sycl_mul_mat_vec_q_id_reorder(
    ...
    size_t             dst_tok_stride,
    size_t             src1_tok_stride) {
    const int ntok = n_tokens > 0 ? n_tokens : 1;
    switch (src0_type) {
        case GGML_TYPE_Q4_K:
            launch_mul_mat_vec_q_moe_reorder<reorder_vec_dot_q_sycl<GGML_TYPE_Q4_K>>(
                vx_base, vy, ids_dev, dst_base, ncols, nrows, n_experts_used,
                expert_weight_stride, dst_row_stride, src1_row_stride,
                ntok, ids_tok_stride, dst_tok_stride, src1_tok_stride, stream);
            return true;
        case GGML_TYPE_Q5_K:
