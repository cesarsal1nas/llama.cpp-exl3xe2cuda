...
static std::vector<llama_device_memory_data> common_get_device_memory_data_impl(
        ...
        uint32_t * hp_n_ctx_default = nullptr) {
    struct user_data_t {
        struct {
    ...
    hp_n_ctx_train = (uint32_t) llama_model_n_ctx_train(model);
    if (hp_n_ctx_default) {
        *hp_n_ctx_default = (uint32_t) llama_model_n_ctx_default(model);
    }
...
static void common_params_fit_impl(
    ...
    uint32_t hp_ncd = 0;
    dmds_t dmds_full = common_get_device_memory_data_impl(path_model, mparams, cparams, devs, hp_ngl, hp_nct, hp_nex, log_level, &hp_ncd);

    uint32_t n_ctx_fit = hp_nct;
    if (n_ctx_auto && n_ctx_min != UINT32_MAX && hp_ncd > 0 && hp_ncd < hp_nct) {
        n_ctx_fit = hp_ncd;
        LOG_INF("%s: n_ctx unset, using default %u (train %u); pass -c 0 for train ctx\n",
                __func__, n_ctx_fit, hp_nct);
    }

    // saturate instead of overflowing, this also preserves the UINT32_MAX sentinel of n_ctx_min:
    const uint32_t n_ctx_max       = (uint32_t) std::min<uint64_t>(uint64_t(n_ctx_fit) * n_streams, UINT32_MAX);
