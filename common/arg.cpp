...
common_params_context common_params_parser_init(common_params & params, llama_example ex, void(*print_usage)(int, char **)) {
    ...
    add_opt(common_arg(
        {"--spec-mtp-cheap"},
        "ExL-style one extra MTP token (forces n-max=1). Same greedy target verify as default draft-mtp; does not skip the check. Use for the cheap n=1 round. Default draft-mtp on llama.cpp EXL3 is still n-max=2: a 2-token verify is almost 2x a 1-token step here, so n-max=2 is faster (~63 vs ~56 TG HTTP). ExL n=1 is ~84 on the same prompt because their q=2 batch is ~1.2x q=1. This flag will not match 84 until that batch cost matches.",
        [](common_params & params) {
            params.speculative.draft.mtp_cheap = true;
