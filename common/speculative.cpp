...
// reject-q sampler not in this fork. Default tau=0 (exact-match accept).
static float common_spec_reject_tau() { return 0.0f; }
static void common_spec_draft_q_clear() {}
static void common_spec_draft_q_push(const llama_token_data_array *) {}
static llama_token common_spec_sample_from_q(const llama_token_data_array *, float) { return 0; }

// Draft-token id histogram for the vocab-shortlist gate.
static uint64_t g_draft_id_n      = 0;
static uint64_t g_draft_id_lt8k   = 0;
static uint64_t g_draft_id_lt16k  = 0;
static uint64_t g_draft_id_lt32k  = 0;
static uint64_t g_draft_id_lt64k  = 0;
static uint64_t g_draft_id_lt80k  = 0;
static uint64_t g_draft_id_lt96k  = 0;
static uint64_t g_draft_id_lt128k = 0;
static llama_token g_draft_id_max = 0;
static std::vector<uint64_t> g_draft_id_hist;

static void spec_hist_init_once() {
    static bool once = false;
    if (once) {
        return;
    }
    once = true;
    const char * p = getenv("LLAMA_DRAFT_ID_HIST");
    if (p && p[0] != '\0') {
        g_draft_id_hist.assign(262144, 0);
    }
}

static void spec_record_draft_id(llama_token id) {
    spec_hist_init_once();
    g_draft_id_n++;
    if (id > g_draft_id_max) {
        g_draft_id_max = id;
    }
    if (id < 8192) {
        g_draft_id_lt8k++;
    ...
    if (!g_draft_id_hist.empty() && id >= 0 && (size_t) id < g_draft_id_hist.size()) {
        g_draft_id_hist[(size_t) id]++;
    }
}

static void spec_dump_draft_id_hist() {
    const char * p = getenv("LLAMA_DRAFT_ID_HIST");
    if (!p || p[0] == '\0' || g_draft_id_hist.empty()) {
        return;
    }
    std::ofstream out(p);
    if (!out) {
        SPC_ERR("failed to write LLAMA_DRAFT_ID_HIST=%s\n", p);
        return;
    }
    uint64_t nz = 0;
    for (size_t i = 0; i < g_draft_id_hist.size(); ++i) {
        if (g_draft_id_hist[i] == 0) {
            continue;
        }
        out << i << '\t' << g_draft_id_hist[i] << '\n';
        nz++;
    }
    SPC_INF("draft-id hist dumped n=%" PRIu64 " nz=%" PRIu64 " -> %s\n", g_draft_id_n, nz, p);
}

static bool spec_env_flag(const char * name, bool def) {
...
static bool spec_fused_draft0() {
    static const bool on = spec_env_flag("LLAMA_MTP_FUSED_DRAFT0", false);
    return on;
}

// Pad process/fused ctx_dft submits to n_max+1 (pads first, logits on last).
// Do not pad 1-token draft steps. Default off: dummy MTP tokens cost more
// than a graph rebuild on this pack.
static bool spec_pad_shape() {
    static const bool on = spec_env_flag("MTP_PAD_SHAPE", false);
    return on;
}

static llama_pos spec_pad_pos0(llama_context * ctx) {
    const uint32_t n_ctx = llama_n_ctx(ctx);
    return (llama_pos) (n_ctx > 16 ? n_ctx - 8 : 0);
}

static void spec_pad_batch_to(
        llama_batch & batch,
        int32_t n_want,
        int32_t n_embd,
        size_t row_bytes,
        llama_context * ctx) {
    if (n_want <= 0 || batch.n_tokens <= 0 || batch.n_tokens > n_want) {
        return;
    }
    const int32_t n_real = batch.n_tokens;
    std::vector<llama_token> toks((size_t) n_real);
    std::vector<llama_pos> poss((size_t) n_real);
    std::vector<float> emb((size_t) n_real * n_embd);
    for (int32_t i = 0; i < n_real; ++i) {
        toks[i] = batch.token[i];
        poss[i] = batch.pos[i];
        std::memcpy(emb.data() + (size_t) i * n_embd, batch.embd + (size_t) i * n_embd, row_bytes);
    }
    common_batch_clear(batch);
    const llama_pos p0 = spec_pad_pos0(ctx);
    const llama_token pad_tok = toks.back();
    const float * pad_h = emb.data() + (size_t) (n_real - 1) * n_embd;
    for (int32_t k = 0; k < n_want - n_real; ++k) {
        common_batch_add(batch, pad_tok, p0 + k, { 0 }, false);
        std::memcpy(batch.embd + (size_t) (batch.n_tokens - 1) * n_embd, pad_h, row_bytes);
    }
    for (int32_t i = 0; i < n_real; ++i) {
        common_batch_add(batch, toks[i], poss[i], { 0 }, i == n_real - 1);
        std::memcpy(batch.embd + (size_t) (batch.n_tokens - 1) * n_embd,
                emb.data() + (size_t) i * n_embd, row_bytes);
    }
}

static void spec_rm_pads(llama_context * ctx) {
    auto * mem = llama_get_memory(ctx);
    const llama_pos p0 = spec_pad_pos0(ctx);
    for (int k = 0; k < 8; ++k) {
        llama_memory_seq_rm(mem, 0, p0 + k, p0 + k + 1);
    }
}

// Draft-head width S. Unset -> 65536. EXL3 uses prefix GEMV (no dump).
// MTP_HEAD_S=0 = full head. MTP_HEAD_Q6K=1 forces the Q6_K dump.
static int32_t spec_head_shortlist() {
    const char * e = getenv("MTP_HEAD_S");
    if (!e || e[0] == '\0') {
        e = getenv("LLAMA_DRAFT_VOCAB_SHORTLIST");
    }
    if (!e || e[0] == '\0') {
        return 65536;
    }
...
// Catch-up process() never reads draft logits. Force the MTP graph to drop the
// Q6_K head even when n_outputs is forced >0 (backend sampler / reserve).
struct mtp_skip_lm_head_guard {
    llama_context * ctx;
    explicit mtp_skip_lm_head_guard(llama_context * c) : ctx(c) {
        if (!ctx) {
            return;
        }
        llama_set_mtp_skip_lm_head(ctx, true);
        static bool logged = false;
        if (!logged) {
            SPC_INF("%s", "MTP process() skip lm_head flag on\n");
            logged = true;
        }
    }
    ~mtp_skip_lm_head_guard() {
        if (!ctx) {
            return;
        }
        llama_set_mtp_skip_lm_head(ctx, false);
    }
};
...
struct common_speculative_impl {
    ...
    virtual void set_sampled(llama_seq_id /*seq_id*/, const llama_tokens & /*ids*/) {}
...
struct common_speculative_impl_draft_mtp : public common_speculative_impl {
    ...
    bool    exl3_keep_kv  = false;   // EXL3 GGUF only: keep drafted MTP KV, skip catch-up
    ...
    // process() catch-up + draft-0 in one ctx_dft submit when the target
    // backend-sampled during decode (llama_get_sampled_token_ith).
    struct fused_draft0_state {
        bool        valid     = false;
        int32_t     n_acc     = -1;
    ...
    std::vector<fused_draft0_state> fused;
    std::vector<int32_t>            accept_n;
    std::vector<float>              draft_reach;
    std::vector<llama_tokens>       sampled_verify;
    float                           dyn_conf = 0.0f;
    int64_t                         t_dft_decode_us = 0;
    int32_t                         n_dft_decode = 0;
        ...
        , params(params.draft)
    {
        ...
        if (this->params.mtp_cheap) {
            this->params.n_max = 1;
            SPC_INF("MTP cheap: n_max=1 p_min=%.2f (HTTP 0.35 ~71 TG vs 0.55 ~68, same greedy verify)\n",
                    this->params.p_min);
        }

        SPC_INF("%s", "adding speculative implementation 'draft-mtp'\n");
        {
            const char * e = getenv("MTP_DRAFT_CONF");
            if (e && e[0] != '\0') {
                dyn_conf = (float) atof(e);
            }
        }
        SPC_INF("- n_max=%d, n_min=%d, p_min=%.2f, dyn_conf=%.2f, n_embd=%d, backend_sampling=%d\n",
                this->params.n_max, this->params.n_min, this->params.p_min, dyn_conf, n_embd, (int) this->params.backend_sampling);
        SPC_INF("%s", "draft path: top_k=10 then argmax; dyn_conf off unless MTP_DRAFT_CONF; p_min gate\n");
        {
            const float tau = common_spec_reject_tau();
            if (tau > 0.0f) {
                SPC_INF("spec-reject draft sample tau=%.3f (LLAMA_SPEC_REJECT)\n", (double) tau);
            }
        }
        SPC_INF("- gpu_layers=%d, cache_k=%s, cache_v=%s, ctx_tgt=%s, ctx_dft=%s, devices=[%s]\n",
                this->params.n_gpu_layers,
                ggml_type_name(this->params.cache_type_k),
                ggml_type_name(this->params.cache_type_v),
                ctx_tgt ? "yes" : "no",
        ...
        llama_set_embeddings_nextn(ctx_tgt, true, /*masked*/ false);
        if (this->params.mtp_cheap) {
            // n_max=1 never consumes MTP h for a second draft step
        ...
        {
            const int32_t sl = spec_head_shortlist();
            if (sl > 0) {
                const bool ok = llama_mtp_init_head_shortlist(ctx_dft, sl);
                SPC_INF("mtp head shortlist S=%d init=%s\n", (int) sl, ok ? "ok" : "FAIL");
                if (!ok) {
                    SPC_ERR("mtp head shortlist init failed S=%d; using full head\n", (int) sl);
                }
            }
        }
        ...
        exl3_keep_kv  = llama_model_has_exl3(llama_get_model(ctx_tgt)) &&
                !is_mem_shared && n_mtp_layers == 1;
        if (exl3_keep_kv) {
            SPC_INF("%s", "EXL3 MTP: keep draft KV, skip process() catch-up (ExL-style)\n");
        }
        ...
        fused.assign(n_seq, {});
        accept_n.assign(n_seq, -1);
        draft_reach.assign(n_seq, 1.0f);
        sampled_verify.assign(n_seq, {});
        if (spec_fused_draft0()) {
            SPC_INF("%s", "MTP fused draft-0 on (LLAMA_MTP_FUSED_DRAFT0=0 to disable)\n");
        }
        if (spec_pad_shape()) {
            SPC_INF("%s", "MTP pad-shape on process/fused (MTP_PAD_SHAPE=0 to disable)\n");
        }
    }

    ~common_speculative_impl_draft_mtp() override {
        if (n_dft_decode > 0) {
            SPC_INF("mtp dft llama_decode n=%d avg=%.2f ms total=%.1f ms\n",
                    (int) n_dft_decode,
                    (t_dft_decode_us / 1000.0) / (double) n_dft_decode,
                    t_dft_decode_us / 1000.0);
        }
        if (spec_fused_draft0() && !fused.empty()) {
            SPC_INF("fused draft-0 used=%" PRIu64 " fallback=%" PRIu64 "\n",
                    fused[0].n_used, fused[0].n_fallback);
        }
        auto * ctx_dft = this->params.ctx_dft;
        for (llama_seq_id seq_id = 0; seq_id < (llama_seq_id) backend_chains.size(); ++seq_id) {
            if (backend_chains[seq_id] == nullptr) {
    ...
    void set_sampled(llama_seq_id seq_id, const llama_tokens & ids) override {
        if (seq_id < 0 || seq_id >= (llama_seq_id) n_seq) {
            return;
        }
        sampled_verify[seq_id] = ids;
    }
    ...
    bool process(const llama_batch & batch_in) override {
        ...
        // EXL3: drafted MTP KV is kept (speculative-simple / server skip the wipe).
        // Catch-up would replay those tokens. Prompt process() still decodes.
        // Server process() runs before sample, so sampled_verify is empty and
        // n_max=2 draft KV already overlaps the verify batch (M-RoPE X>=Y).
        llama_pos y = -1;
        if (n_seq == 1 && i_batch_beg[0] >= 0 && batch_in.pos) {
            y = batch_in.pos[i_batch_beg[0]];
        }
        const llama_pos x = llama_memory_seq_pos_max(llama_get_memory(ctx_dft), /*seq*/ 0);
        const bool skip_exl3_catchup =
            exl3_keep_kv && n_seq == 1 &&
            (!sampled_verify[0].empty() || (x >= 0 && y >= 0 && x >= y));
        if (skip_exl3_catchup) {
            sampled_verify[0].clear();
        }
        if (!is_mem_shared && !skip_exl3_catchup) {
            bool fused_ok = false;
            // Single-head MTP only. Prefill ubatches stay on the legacy logits=0 path.
            if (spec_fused_draft0() && !chain_heads && n_seq == 1 &&
                    n_tokens >= 2 && n_tokens <= this->params.n_max + 1 &&
                    i_batch_beg[0] >= 0) {
                const llama_seq_id seq_id = 0;
                const int32_t beg = i_batch_beg[seq_id];
                const int32_t n_rows = i_batch_end[seq_id] - beg + 1;
                llama_token bonus = LLAMA_TOKEN_NULL;
                int32_t n_acc = -1;
                if (!sampled_verify[seq_id].empty()) {
                    n_acc = std::min((int32_t) sampled_verify[seq_id].size() - 1, n_rows - 1);
                    bonus = sampled_verify[seq_id].back();
                    sampled_verify[seq_id].clear();
                } else {
                    std::vector<llama_token> sampled((size_t) n_rows, LLAMA_TOKEN_NULL);
                    bool have_sampled = true;
                    for (int32_t k = 0; k < n_rows; ++k) {
                        sampled[k] = llama_get_sampled_token_ith(ctx_tgt, beg + k);
                        if (sampled[k] == LLAMA_TOKEN_NULL) {
                            have_sampled = false;
                            break;
                        }
                    }
                    if (have_sampled) {
                        n_acc = 0;
                        while (n_acc < n_rows - 1 && sampled[n_acc] == batch_in.token[beg + 1 + n_acc]) {
                            n_acc++;
                        }
                        bonus = sampled[n_acc];
                    }
                }
                if (n_acc >= 0 && bonus != LLAMA_TOKEN_NULL) {
                    common_batch_clear(batch);
                    for (int32_t k = 0; k <= n_acc; ++k) {
                        common_batch_add(batch, batch_in.token[beg + k], batch_in.pos[beg + k], { seq_id }, false);
                    }
                    const llama_pos pos_d0 = batch_in.pos[beg] + n_acc + 1;
                    common_batch_add(batch, bonus, pos_d0, { seq_id }, true);

                    std::memcpy(batch.embd, pending_h[seq_id].data(), row_bytes);
                    for (int32_t k = 1; k <= n_acc; ++k) {
                        const float * h = llama_get_embeddings_nextn_ith(ctx_tgt, beg + k - 1);
                        std::memcpy(batch.embd + (size_t) k * n_embd, h, row_bytes);
                    }
                    {
                        const float * h_d0 = llama_get_embeddings_nextn_ith(ctx_tgt, beg + n_acc);
                        std::memcpy(batch.embd + (size_t) (n_acc + 1) * n_embd, h_d0, row_bytes);
                    }

                    const bool pad = spec_pad_shape() && !chain_heads && n_seq == 1;
                    if (pad) {
                        spec_pad_batch_to(batch, this->params.n_max + 1, n_embd, row_bytes, ctx_dft);
                    }
                    const int32_t rc = llama_decode(ctx_dft, batch);
                    if (pad) {
                        spec_rm_pads(ctx_dft);
                    }
                    if (rc == 0) {
                        fused[seq_id].valid   = true;
                        fused[seq_id].n_acc   = n_acc;
                        fused[seq_id].id_last = bonus;
                        fused[seq_id].n_past  = pos_d0;
                        fused[seq_id].i_last  = batch.n_tokens - 1;
                        fused_ok = true;
                    } else {
                        SPC_ERR("fused draft-0 llama_decode failed rc=%d n_acc=%d n_tokens=%d\n",
                                (int) rc, (int) n_acc, (int) n_tokens);
                    }
                }
            }
            if (!fused_ok) {
            ...
            if (fused_ok) {
                // fused submit already ran; fall through to stash verify_h
            } else {
            int32_t n_take = n_tokens;
            if (n_seq == 1 && !sampled_verify[0].empty() && i_batch_beg[0] >= 0) {
                const int32_t n_acc = std::max(0, (int32_t) sampled_verify[0].size() - 1);
                n_take = std::min(n_tokens, i_batch_beg[0] + n_acc + 1);
                sampled_verify[0].clear();
            }
            common_batch_clear(batch);
            ...
            const bool pad = spec_pad_shape() && !chain_heads && n_seq == 1 &&
                    n_take <= this->params.n_max + 1;
            mtp_skip_lm_head_guard skip_head(pad ? nullptr : ctx_dft);
            ...
            // TODO:this is generally true, but would be nice to assert it
            {
                ...
                if (n_take > 1) {
                    std::memcpy(batch.embd + (size_t) 1 * n_embd, h_tgt, row_bytes * (n_take - 1));
                }
            }
    ...
    void draft(common_speculative_draft_params_vec & dparams) override {
        ...
        for (llama_seq_id seq_id = 0; seq_id < (llama_seq_id) n_seq; ++seq_id) {
            ...
            n_drafting++;
            drafting[seq_id] = true;
            draft_reach[seq_id] = 1.0f;
            common_sampler_reset(smpls[seq_id].get());

            auto & fs = fused[seq_id];
            const bool fuse_hit = fs.valid &&
                    fs.n_acc == accept_n[seq_id] &&
                    fs.id_last == dp.id_last &&
                    fs.n_past == dp.n_past &&
                    fs.i_last >= 0;

            if (fuse_hit) {
                i_last[seq_id] = fs.i_last;
                fs.n_used++;
                ...
                if (fs.valid) {
                    fs.n_fallback++;
                    auto * mem_dft = llama_get_memory(ctx_dft);
                    llama_memory_seq_rm(mem_dft, seq_id, fs.n_past, -1);
                    fs.valid = false;
                    if (fs.n_fallback <= 4 || (fs.n_fallback % 64) == 0) {
                        SPC_INF("fused draft-0 fallback seq=%d n_acc=%d accept_n=%d id %d vs %d pos %d vs %d (n_fallback=%" PRIu64 ")\n",
                                (int) seq_id, (int) fs.n_acc, (int) accept_n[seq_id],
                                (int) fs.id_last, (int) dp.id_last,
                                (int) fs.n_past, (int) dp.n_past, fs.n_fallback);
                    }
                }
                common_batch_add(batch, dp.id_last, dp.n_past, { seq_id }, true);
        ...
        int i = 0;
        bool need_decode = !skip_first_decode;
        ...
        while (n_drafting > 0) {
            ...
            if (need_decode) {
                ...
                int ret = llama_decode(ctx_dft, batch);
                t_dft_decode_us += ggml_time_us() - t0;
                n_dft_decode++;
                if (ret != 0) {
                    SPC_ERR("llama_decode[%d] returned %d\n", i, ret);
                    break;
            ...
            for (llama_seq_id seq_id = 0; seq_id < (llama_seq_id) n_seq; ++seq_id) {
                ...
                // add drafted token for each sequence
                const float tau = common_spec_reject_tau();
                llama_token id = (tau > 0.0f)
                    ? common_spec_sample_from_q(cur_p, tau)
                    : cur_p->data[0].id;
                ...
                if (dyn_conf > 0.0f) {
                    draft_reach[seq_id] *= cur_p->data[0].p;
                }

                common_spec_draft_q_push(cur_p);
                id = llama_mtp_remap_draft_id(ctx_dft, id);
                common_sampler_accept(smpl, id, true);
                ...
                result.push_back(id);
                spec_record_draft_id(id);

                if (params.n_max <= (int) result.size() ||
                        (dyn_conf > 0.0f && draft_reach[seq_id] < dyn_conf)) {
                    drafting[seq_id] = false;
                    n_drafting--;
                    continue;
                }
    ...
    void accept(llama_seq_id seq_id, uint16_t n_accepted, bool /*is_other*/) override {
        ...
        accept_n[seq_id] = (int32_t) n_accepted;
...
common_speculative_init_result::common_speculative_init_result(
    ...
    if (spec_mtp) {
        ...
        if (!std::getenv("GGML_MTP_DRAFT_NUBATCH")) {
            const uint32_t n_ub = std::max(
                    (uint32_t) params.n_ubatch,
                    (uint32_t) std::max(8, params.speculative.draft.n_max + 2));
            cparams.n_ubatch = n_ub;
            LOG_INF("%s: MTP draft n_ubatch=%u (catch-up batch; decode still 1-2 tok)\n",
                    __func__, n_ub);
        }
    }
    ...
    if (const char * e = std::getenv("GGML_MTP_DRAFT_NCTX")) {
        const uint32_t n = (uint32_t) atoi(e);
        if (n >= 256) {
            LOG_INF("%s: GGML_MTP_DRAFT_NCTX overrides draft n_ctx %u -> %u\n", __func__, cparams.n_ctx, n);
            cparams.n_ctx = n;
        }
    }
    if (const char * e = std::getenv("GGML_MTP_DRAFT_NUBATCH")) {
        const uint32_t n = (uint32_t) atoi(e);
        if (n >= 1) {
            LOG_INF("%s: GGML_MTP_DRAFT_NUBATCH overrides draft n_ubatch/n_batch %u/%u -> %u\n",
                    __func__, cparams.n_ubatch, cparams.n_batch, n);
            cparams.n_ubatch = n;
            cparams.n_batch  = std::max(cparams.n_batch, n);
        }
    }
...
void common_speculative_set_sampled(common_speculative * spec, llama_seq_id seq_id, const llama_tokens & ids) {
    if (spec == nullptr) {
        return;
    }
    for (auto & impl : spec->impls) {
        impl->set_sampled(seq_id, ids);
    }
}
...
void common_speculative_print_stats(const common_speculative * spec) {
    ...
    for (const auto & impl : spec->impls) {
        ...
        SPC_INF("statistics %16s: #calls(b,g,a) = %4zu %6zu %6zu, #gen drafts = %6zu, #acc drafts = %5zu, #gen tokens = %6zu, #acc tokens = %5zu%s%s\n",
                ...
                str_perf.c_str());
        if (g_draft_id_n > 0) {
            const double p8   = 100.0 * (double) g_draft_id_lt8k   / (double) g_draft_id_n;
            const double p16  = 100.0 * (double) g_draft_id_lt16k  / (double) g_draft_id_n;
            const double p32  = 100.0 * (double) g_draft_id_lt32k  / (double) g_draft_id_n;
            const double p64  = 100.0 * (double) g_draft_id_lt64k  / (double) g_draft_id_n;
            const double p80  = 100.0 * (double) g_draft_id_lt80k  / (double) g_draft_id_n;
            const double p96  = 100.0 * (double) g_draft_id_lt96k  / (double) g_draft_id_n;
            const double p128 = 100.0 * (double) g_draft_id_lt128k / (double) g_draft_id_n;
            SPC_INF("draft-id histogram: n=%" PRIu64 " max=%d id<8k=%.1f%% id<16k=%.1f%% id<32k=%.1f%% id<64k=%.1f%% id<80k=%.1f%% id<96k=%.1f%% id<128k=%.1f%%\n",
                    g_draft_id_n, (int) g_draft_id_max, p8, p16, p32, p64, p80, p96, p128);
            spec_dump_draft_id_hist();
        }
    }
}
