...
struct ggml_tensor * llama_model_loader::create_tensor(
    ...
    auto buft_for_tensor = [&](ggml_tensor * t_meta) -> ggml_backend_buffer_type_t {
        ...
        if (info.op == GGML_OP_NONE || (flags & TENSOR_SKIP)) {
            ...
            // EXL3 companions are not create_tensor'd; count them when the weight is skipped
            if (ggml_is_exl3(t_meta->type)) {
                const std::string wname = tn.str();
                const auto dot = wname.rfind('.');
                const std::string stem = (dot == std::string::npos) ? wname + "." : wname.substr(0, dot + 1);
                for (const char * suf : { "suh", "svh" }) {
                    const std::string cname = stem + suf;
                    if (const auto * cw = get_weight(cname.c_str())) {
                        size_data -= ggml_nbytes(cw->tensor);
                        n_created++;
                    }
                }
            }
