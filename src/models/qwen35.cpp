...
llama_model_qwen35::graph_mtp::graph_mtp(const llama_model & model, const llm_graph_params & params)
    ...
    ggml_tensor * inp_pos = build_inp_pos();
    static const bool skip_empty_head = []() {
        const char * e = getenv("LLAMA_MTP_SKIP_EMPTY_HEAD");
        return !e || e[0] == '\0' || e[0] != '0';
    }();
    const bool need_head = !skip_empty_head ||
            (!cparams.mtp_skip_lm_head && n_outputs > 0);
    ggml_tensor * inp_out_ids = need_head ? build_inp_out_ids() : nullptr;
    ...
    GGML_ASSERT(inp_out_ids && "MTP lm_head needs out_ids");
    cur = ggml_get_rows(ctx0, cur, inp_out_ids);
    ...
    // First-S cols. Default keeps logits width S (ids 0..S-1). MTP_HEAD_PAD=1
    // or MTP_HEAD_NOPAD=0 concat -inf to n_vocab.
    if (mtp_head_shortlist || mtp_head_prefix_n > 0) {
        ggml_tensor * sl  = mtp_head_shortlist;
        ggml_tensor * pad = mtp_head_pad_neg;
        if (sl) {
            GGML_ASSERT(sl->ne[0] == head_w->ne[0]);
            cur = build_lora_mm(sl, cur, head_s);
        } else {
            cur = build_lora_mm(head_w, cur, head_s, mtp_head_prefix_n);
        }
        static const bool use_pad = []() {
            const char * p = getenv("MTP_HEAD_PAD");
            if (p && p[0] != '\0' && p[0] != '0') {
                return true;
            }
            const char * e = getenv("MTP_HEAD_NOPAD");
            return e && e[0] == '0';
        }();
        if (use_pad && pad && cur->ne[1] > 0) {
            ggml_tensor * pad_use = pad;
            if (cur->ne[1] != pad->ne[1]) {
                pad_use = ggml_repeat_4d(ctx0, pad, pad->ne[0], cur->ne[1], 1, 1);
            }
            cur = ggml_concat(ctx0, cur, pad_use, /*dim=*/ 0);
        }
    } else {
