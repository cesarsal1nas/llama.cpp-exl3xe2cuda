...
class llm_graph_input_exl3_h128 : public llm_graph_input_i {
public:
    llm_graph_input_exl3_h128(const float * data) : data(data) {}
    virtual ~llm_graph_input_exl3_h128() = default;

    void set_input(const llama_ubatch * ubatch) override;

    bool can_reuse(const llm_graph_params & params) override;

    ggml_tensor * h128 = nullptr; // F32 [128, 128]
...
struct llm_graph_params {
    ...
    // Draft MTP lm_head shortlist (owned by llama_context). Null = full head.
    ggml_tensor * mtp_head_shortlist = nullptr;
    ggml_tensor * mtp_head_pad_neg   = nullptr;
    int64_t       mtp_head_prefix_n  = 0;
    ...
    bool allow_reuse(const llm_graph_params & other) const {
        ...
        return
            ...
            cparams.mtp_skip_lm_head        == other.cparams.mtp_skip_lm_head        &&
            ...
            loras == other.loras &&
            cross == other.cross &&
            mtp_head_shortlist              == other.mtp_head_shortlist              &&
            mtp_head_pad_neg                == other.mtp_head_pad_neg                &&
            mtp_head_prefix_n               == other.mtp_head_prefix_n;
    }
};
...
struct llm_graph_context {
    ...
    mutable ggml_tensor * inp_exl3_h128 = nullptr;
    ...
    ggml_tensor * get_exl3_h128() const;
