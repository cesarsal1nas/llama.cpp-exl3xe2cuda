...
llama_context::llama_context(
    ...
    cparams.mtp_skip_lm_head        = false;
    ...
    if (params.n_ctx == 0) {
        cparams.n_ctx = llama_model_n_ctx_default(&model);
        if (cparams.n_ctx != hparams.n_ctx_train) {
            LLAMA_LOG_INFO("%s: n_ctx unset, default %u (train %u); pass -c 0 for train ctx\n",
...
void llama_context::set_mtp_skip_lm_head(bool value) {
    cparams.mtp_skip_lm_head = value;
}
...
bool llama_context::init_mtp_head_shortlist(int32_t n_short) {
    mtp_head_shortlist = nullptr;
    mtp_head_pad_neg   = nullptr;
    mtp_head_prefix_n  = 0;
    mtp_head_perm.clear();
    buf_mtp_sl.reset();
    ctx_mtp_sl.reset();
    ...
    const int il = (int) model.hparams.n_layer() + cparams.nextn_layer_offset;
    if (il < 0 || il >= (int) model.layers.size()) {
        LLAMA_LOG_ERROR("%s: MTP layer index %d out of range (layers=%zu)\n",
                __func__, il, model.layers.size());
        return false;
    }

    const auto & layer = model.layers[il];
    ggml_tensor * src = layer.nextn.shared_head_head ? layer.nextn.shared_head_head : model.output;
    if (!src || !src->buffer) {
        LLAMA_LOG_ERROR("%s: no draft lm_head tensor to gather\n", __func__);
        return false;
    }

    const int64_t n_embd  = src->ne[0];
    const int64_t n_vocab = src->ne[1];
    const char * q6k = getenv("MTP_HEAD_Q6K");
    const bool force_q6k = q6k && q6k[0] != '\0' && q6k[0] != '0';
    if ((int64_t) n_short >= n_vocab) {
        LLAMA_LOG_WARN("%s: S=%d >= n_vocab=%d, leaving full head\n",
                __func__, n_short, (int) n_vocab);
        return true;
    }

    // EXL3 / BF16 / F16 default: prefix GEMV (first S cols). No -inf concat.
    // MTP_HEAD_PAD=1 or MTP_HEAD_NOPAD=0 restores the pad. MTP_HEAD_Q6K=1 dumps Q6_K.
    if ((ggml_is_exl3(src->type) || src->type == GGML_TYPE_BF16 ||
            src->type == GGML_TYPE_F16 || src->type == GGML_TYPE_F32) && !force_q6k) {
        int32_t s = n_short;
        if (ggml_is_exl3(src->type) && s % 256) {
            s = (s + 255) & ~255;
        }
        if ((int64_t) s >= n_vocab) {
            LLAMA_LOG_INFO("%s: EXL3 prefix S=%d >= n_vocab, full head\n", __func__, s);
            return true;
        }
        const char * pad_on = getenv("MTP_HEAD_PAD");
        const char * nopad  = getenv("MTP_HEAD_NOPAD");
        const bool want_pad = (pad_on && pad_on[0] != '\0' && pad_on[0] != '0') ||
                (nopad && nopad[0] == '0');
        if (want_pad) {
            ...
            ggml_tensor * pad = ggml_new_tensor_2d(ctx, GGML_TYPE_F32, n_vocab - s, 1);
            ggml_set_name(pad, "mtp_head_pad_neg");
            ggml_backend_buffer_type_t buft = ggml_backend_buffer_get_type(src->buffer);
            ggml_backend_buffer_t buf = ggml_backend_alloc_ctx_tensors_from_buft(ctx, buft);
            if (!buf) {
                LLAMA_LOG_ERROR("%s: failed to allocate prefix pad on %s\n",
                        __func__, ggml_backend_buft_name(buft));
                ggml_free(ctx);
                return false;
            }
            std::vector<float> pad_h((size_t) (n_vocab - s), -1e30f);
            ggml_backend_tensor_set(pad, pad_h.data(), 0, pad_h.size() * sizeof(float));
            ctx_mtp_sl.reset(ctx);
            buf_mtp_sl.reset(buf);
            mtp_head_pad_neg = pad;
        }
        mtp_head_prefix_n = s;
        sched_need_reserve = true;
        LLAMA_LOG_INFO("%s: prefix GEMV type=%s S=%d / %d pad=%d (MTP_HEAD_PAD=1 for concat)\n",
                __func__, ggml_type_name(src->type), s, (int) n_vocab, (int) want_pad);
        return true;
    }

    const size_t row_q6 = ggml_row_size(GGML_TYPE_Q6_K, n_embd);
    const auto * tq6 = ggml_get_type_traits(GGML_TYPE_Q6_K);
    if (!tq6 || !tq6->to_float) {
        LLAMA_LOG_ERROR("%s: missing Q6_K type traits\n", __func__);
        return false;
    }

    std::vector<uint8_t> src_h((size_t) n_short * row_q6);
    std::vector<float>   f32((size_t) n_embd);

    auto row0_rms = [&](const uint8_t * row) -> double {
        tq6->to_float(row, f32.data(), n_embd);
        double ss = 0.0;
        for (int64_t i = 0; i < n_embd; ++i) {
            ss += (double) f32[i] * (double) f32[i];
        }
        return std::sqrt(ss / (double) n_embd);
    };

    double rms0 = 0.0;
    bool soa = false;

    if (ggml_is_exl3(src->type)) {
        if (!tq6->from_float_ref) {
            LLAMA_LOG_ERROR("%s: missing Q6_K from_float_ref\n", __func__);
            return false;
        }
        if (n_embd % 128 != 0 || n_vocab % 128 != 0) {
            LLAMA_LOG_ERROR("%s: EXL3 lm_head shape %ldx%ld not 128-aligned\n",
                    __func__, (long) n_embd, (long) n_vocab);
            return false;
        }
        const auto scales = model.get_exl3_scales(src);
        if (!scales.first || !scales.second) {
            LLAMA_LOG_ERROR("%s: EXL3 lm_head missing suh/svh\n", __func__);
            return false;
        }
        int64_t n_cols = ((int64_t) n_short + 127) & ~((int64_t) 127);
        if (n_cols > n_vocab) {
            n_cols = n_vocab;
        }
        if (n_cols % 128 != 0 || (int64_t) n_short > n_cols) {
            LLAMA_LOG_ERROR("%s: cannot pad shortlist S=%d to 128 (n_vocab=%ld)\n",
                    __func__, n_short, (long) n_vocab);
            return false;
        }
        LLAMA_LOG_INFO("%s: EXL3_%d reconstruct_had prefix K=%ld N_cols=%ld -> Q6_K S=%d\n",
                __func__, ggml_exl3_n_bits(src->type),
                (long) n_embd, (long) n_cols, n_short);
        std::vector<ggml_fp16_t> host_f16((size_t) n_embd * (size_t) n_cols);
#ifdef GGML_USE_CUDA
        if (!ggml_cuda_exl3_reconstruct_had_prefix_f16(
                    src, scales.first, scales.second, n_cols, host_f16.data())) {
            LLAMA_LOG_ERROR("%s: EXL3 reconstruct_had prefix failed\n", __func__);
            return false;
        }
#else
        LLAMA_LOG_ERROR("%s: EXL3 Q6_K dump needs CUDA reconstruct_had; use prefix GEMV\n", __func__);
        return false;
#endif
#ifdef _OPENMP
        #pragma omp parallel
        {
            std::vector<float> local((size_t) n_embd);
            #pragma omp for
            for (int32_t s = 0; s < n_short; ++s) {
                for (int64_t k = 0; k < n_embd; ++k) {
                    local[k] = ggml_fp16_to_fp32(host_f16[(size_t) k * (size_t) n_cols + (size_t) s]);
                }
                tq6->from_float_ref(local.data(), src_h.data() + (size_t) s * row_q6, n_embd);
            }
        }
#else
        for (int32_t s = 0; s < n_short; ++s) {
            for (int64_t k = 0; k < n_embd; ++k) {
                f32[k] = ggml_fp16_to_fp32(host_f16[(size_t) k * (size_t) n_cols + (size_t) s]);
            }
            tq6->from_float_ref(f32.data(), src_h.data() + (size_t) s * row_q6, n_embd);
        }
#endif
        host_f16.clear();
        host_f16.shrink_to_fit();
        rms0 = row0_rms(src_h.data());
        if (!(rms0 > 1e-4 && rms0 < 1e2 && std::isfinite(rms0))) {
            LLAMA_LOG_ERROR("%s: EXL3 shortlist row0 RMS %g garbage; not enabling\n",
                    __func__, rms0);
            return false;
        }
    } else if (src->type != GGML_TYPE_Q6_K) {
        LLAMA_LOG_ERROR("%s: shortlist wants Q6_K or EXL3 src, got %s\n",
                __func__, ggml_type_name(src->type));
        return false;
    }

    const int64_t qk          = ggml_blck_size(GGML_TYPE_Q6_K);
    const size_t  bsz         = ggml_type_size(GGML_TYPE_Q6_K);
    const int64_t nblocks_row = n_embd / qk;
    const int64_t nblocks_s   = (int64_t) n_short * nblocks_row;
    const int64_t nblocks_all = src->type == GGML_TYPE_Q6_K
            ? (int64_t) (ggml_nbytes(src) / bsz)
            : nblocks_s;
    if (qk <= 0 || n_embd % qk != 0 || nblocks_all < nblocks_s) {
        LLAMA_LOG_ERROR("%s: bad Q6_K geometry qk=%ld n_embd=%ld blocks=%ld need=%ld\n",
                __func__, (long) qk, (long) n_embd, (long) nblocks_all, (long) nblocks_s);
        return false;
    }

    if (src->type == GGML_TYPE_Q6_K) {
        const size_t row_src = ggml_row_size(src->type, n_embd);
        if ((size_t) src->nb[1] != row_src) {
            LLAMA_LOG_ERROR("%s: lm_head row stride %zu != packed row %zu\n",
                    __func__, (size_t) src->nb[1], row_src);
            return false;
        }
        // Device output.weight is already Q6_K SoA by MTP ctor (row0 packed-dequant
        // RMS ~1e7). Prefix memcpy is not "first S packed rows". Unpack SoA planes
        // back to packed Q6_K; first mul_mat then SoA-reorders the shortlist.
        ggml_backend_tensor_get(src, src_h.data(), 0, src_h.size());
        rms0 = row0_rms(src_h.data());
        soa = !(rms0 > 1e-4 && rms0 < 1e2 && std::isfinite(rms0));
    }

    const char * rank_e = getenv("MTP_HEAD_RANK");
    const char * map_e  = getenv("MTP_HEAD_MAP");
    const bool rank_on = src->type == GGML_TYPE_Q6_K && rank_e && rank_e[0] != '\0' && rank_e[0] != '0';
    const bool map_on  = src->type == GGML_TYPE_Q6_K && map_e && map_e[0] != '\0';
    std::vector<llama_token> perm((size_t) n_short);
    for (int32_t i = 0; i < n_short; ++i) {
        perm[i] = i;
    }
    int64_t n_pref_overlap = n_short;
    if (map_on) {
        std::ifstream in(map_e);
        if (!in) {
            LLAMA_LOG_ERROR("%s: MTP_HEAD_MAP open failed: %s\n", __func__, map_e);
            return false;
        }
        for (int32_t i = 0; i < n_short; ++i) {
            long v = -1;
            if (!(in >> v) || v < 0 || v >= n_vocab) {
                LLAMA_LOG_ERROR("%s: MTP_HEAD_MAP bad id at %d (got %ld)\n", __func__, i, v);
                return false;
            }
            perm[i] = (llama_token) v;
        }
        n_pref_overlap = 0;
        for (int32_t i = 0; i < n_short; ++i) {
            if (perm[i] < n_short) {
    ...
    const size_t ql_b  = (size_t) qk / 2;
    const size_t qh_b  = (size_t) qk / 4;
    const size_t sc_b  = (size_t) qk / 16;
    const size_t dm_b  = 2;
    const size_t ql_all = ql_b * (size_t) nblocks_all;
    const size_t qh_all = qh_b * (size_t) nblocks_all;
    const size_t sc_all = sc_b * (size_t) nblocks_all;

    if (rank_on || map_on) {
        std::vector<uint8_t> all(ggml_nbytes(src));
        ggml_backend_tensor_get(src, all.data(), 0, all.size());
        if (rank_on && !map_on) {
        std::vector<std::pair<float, llama_token>> sc((size_t) n_vocab);
        if (soa) {
            const uint8_t * dm = all.data() + ql_all + qh_all + sc_all;
            for (int64_t r = 0; r < n_vocab; ++r) {
                float s = 0.0f;
                const uint8_t * row_dm = dm + (size_t) r * (size_t) nblocks_row * dm_b;
                for (int64_t b = 0; b < nblocks_row; ++b) {
                    ggml_fp16_t h;
                    std::memcpy(&h, row_dm + (size_t) b * dm_b, sizeof(h));
                    s += std::fabs(ggml_fp16_to_fp32(h));
                }
                sc[(size_t) r] = { s, (llama_token) r };
            }
        } else {
            for (int64_t r = 0; r < n_vocab; ++r) {
                float s = 0.0f;
                const uint8_t * row = all.data() + (size_t) r * row_q6;
                for (int64_t b = 0; b < nblocks_row; ++b) {
                    const uint8_t * blk = row + (size_t) b * bsz;
                    ggml_fp16_t h;
                    std::memcpy(&h, blk + ql_b + qh_b + sc_b, sizeof(h));
                    s += std::fabs(ggml_fp16_to_fp32(h));
                }
                sc[(size_t) r] = { s, (llama_token) r };
            }
        }
        std::nth_element(sc.begin(), sc.begin() + n_short, sc.end(),
                [](const auto & a, const auto & b) { return a.first > b.first; });
        std::sort(sc.begin(), sc.begin() + n_short,
                [](const auto & a, const auto & b) { return a.first > b.first; });
        n_pref_overlap = 0;
        for (int32_t i = 0; i < n_short; ++i) {
            perm[i] = sc[(size_t) i].second;
            if (perm[i] < n_short) {
                n_pref_overlap++;
            }
        }
        ...
        if (soa) {
            const uint8_t * ql = all.data();
            const uint8_t * qh = all.data() + ql_all;
            const uint8_t * scp = all.data() + ql_all + qh_all;
            const uint8_t * dm = all.data() + ql_all + qh_all + sc_all;
            for (int32_t i = 0; i < n_short; ++i) {
                const int64_t r = perm[i];
                for (int64_t b = 0; b < nblocks_row; ++b) {
                    const int64_t ib_src = r * nblocks_row + b;
                    const int64_t ib_dst = (int64_t) i * nblocks_row + b;
                    uint8_t * blk = src_h.data() + (size_t) ib_dst * bsz;
                    std::memcpy(blk,                        ql + (size_t) ib_src * ql_b, ql_b);
                    std::memcpy(blk + ql_b,                 qh + (size_t) ib_src * qh_b, qh_b);
                    std::memcpy(blk + ql_b + qh_b,          scp + (size_t) ib_src * sc_b, sc_b);
                    std::memcpy(blk + ql_b + qh_b + sc_b,   dm + (size_t) ib_src * dm_b, dm_b);
                }
            }
        } else {
            for (int32_t i = 0; i < n_short; ++i) {
                std::memcpy(src_h.data() + (size_t) i * row_q6,
                        all.data() + (size_t) perm[i] * row_q6, row_q6);
            }
        }
        rms0 = row0_rms(src_h.data());
        if (!(rms0 > 1e-4 && rms0 < 1e2 && std::isfinite(rms0))) {
            LLAMA_LOG_ERROR("%s: rank gather row0 RMS %g garbage; not enabling\n",
                    __func__, rms0);
            return false;
        }
        mtp_head_perm = std::move(perm);
    } else if (soa) {
        std::vector<uint8_t> ql((size_t) nblocks_s * ql_b);
        std::vector<uint8_t> qh((size_t) nblocks_s * qh_b);
        std::vector<uint8_t> sc((size_t) nblocks_s * sc_b);
        std::vector<uint8_t> dm((size_t) nblocks_s * dm_b);
        ggml_backend_tensor_get(src, ql.data(), 0,                         ql.size());
        ggml_backend_tensor_get(src, qh.data(), ql_all,                    qh.size());
        ggml_backend_tensor_get(src, sc.data(), ql_all + qh_all,           sc.size());
        ggml_backend_tensor_get(src, dm.data(), ql_all + qh_all + sc_all,  dm.size());
        for (int64_t ib = 0; ib < nblocks_s; ++ib) {
            uint8_t * blk = src_h.data() + (size_t) ib * bsz;
            std::memcpy(blk,                         ql.data() + (size_t) ib * ql_b, ql_b);
            std::memcpy(blk + ql_b,                  qh.data() + (size_t) ib * qh_b, qh_b);
            std::memcpy(blk + ql_b + qh_b,           sc.data() + (size_t) ib * sc_b, sc_b);
            std::memcpy(blk + ql_b + qh_b + sc_b,    dm.data() + (size_t) ib * dm_b, dm_b);
        }
        rms0 = row0_rms(src_h.data());
        if (!(rms0 > 1e-4 && rms0 < 1e2 && std::isfinite(rms0))) {
            LLAMA_LOG_ERROR("%s: SoA unpack row0 RMS %g still garbage; not enabling\n",
                    __func__, rms0);
            return false;
        }
    }
    ...
    ggml_tensor * head_s = ggml_new_tensor_2d(ctx, GGML_TYPE_Q6_K, n_embd, n_short);
    ggml_set_name(head_s, "mtp_head_shortlist");
    ggml_tensor * pad = ggml_new_tensor_2d(ctx, GGML_TYPE_F32, n_vocab - n_short, 1);
    ggml_set_name(pad, "mtp_head_pad_neg");

    ggml_backend_buffer_type_t buft = ggml_backend_buffer_get_type(src->buffer);
    ggml_backend_buffer_t buf = ggml_backend_alloc_ctx_tensors_from_buft(ctx, buft);
    if (!buf) {
        LLAMA_LOG_ERROR("%s: failed to allocate shortlist buffer on %s\n",
                __func__, ggml_backend_buft_name(buft));
        ggml_free(ctx);
        return false;
    }
    ...
    ggml_backend_tensor_set(head_s, src_h.data(), 0, src_h.size());

    std::vector<float> pad_h((size_t) (n_vocab - n_short), -1e30f);
    ggml_backend_tensor_set(pad, pad_h.data(), 0, pad_h.size() * sizeof(float));

    ctx_mtp_sl.reset(ctx);
    buf_mtp_sl.reset(buf);
    mtp_head_shortlist = head_s;
    mtp_head_pad_neg   = pad;
    sched_need_reserve = true;

    LLAMA_LOG_WARN("%s: draft lm_head shortlist Q6_K S=%d / %d src=%s extra=%p soa=%d rank=%d overlap_prefix=%.2f%% row0_rms=%.5f (%.2f MiB) + -inf pad\n",
            __func__, n_short, (int) n_vocab, src->name, src->extra,
            (int) soa, (int) (rank_on || map_on), 100.0 * (double) n_pref_overlap / (double) n_short,
            rms0, src_h.size() / (1024.0 * 1024.0));
    return true;
}

llama_token llama_context::remap_mtp_draft_id(llama_token id) const {
    if (mtp_head_perm.empty() || id < 0 || (size_t) id >= mtp_head_perm.size()) {
        return id;
    }
    return mtp_head_perm[(size_t) id];
}
...
static void copy_logits_rows(
        ggml_backend_t backend,
        const ggml_tensor * t_logits,
        float * logits_out,
        int32_t n_outputs,
        int64_t n_vocab) {
    const int64_t n_cols = t_logits->ne[0];
    if (n_cols == n_vocab) {
        ggml_backend_tensor_get_async(backend, t_logits, logits_out, 0,
                (size_t) n_outputs * n_vocab * sizeof(float));
        return;
    }
    GGML_ASSERT(n_cols > 0 && n_cols < n_vocab);
    std::vector<float> tmp((size_t) n_outputs * n_cols);
    ggml_backend_tensor_get_async(backend, t_logits, tmp.data(), 0, tmp.size() * sizeof(float));
    ggml_backend_synchronize(backend);
    for (int32_t i = 0; i < n_outputs; ++i) {
        float * dst = logits_out + (int64_t) i * n_vocab;
        memcpy(dst, tmp.data() + (int64_t) i * n_cols, (size_t) n_cols * sizeof(float));
        for (int64_t j = n_cols; j < n_vocab; ++j) {
            dst[j] = -INFINITY;
        }
    }
}
...
int llama_context::encode(const llama_batch & batch_inp) {
    ...
    if (logits.data && t_logits) {
        ...
        copy_logits_rows(backend_res, t_logits, logits.data, (int32_t) n_tokens, n_vocab);
    }
    ...
    do {
        ...
        if (logits.data && t_logits && n_outputs > 0 && needs_raw_logits(ubatch, sampling.samplers)) {
            ...
            if (n_outputs) {
                ...
                copy_logits_rows(backend_res, t_logits, logits_out, n_outputs, n_vocab);
            }
        }
...
llm_graph_params llama_context::graph_params(
    ...
    return {
        ...
        /*.cb          =*/ graph_get_cb(),
        /*.res         =*/ res,
        /*.mtp_head_shortlist =*/ mtp_head_shortlist,
        /*.mtp_head_pad_neg   =*/ mtp_head_pad_neg,
        /*.mtp_head_prefix_n  =*/ mtp_head_prefix_n,
        /*.model              =*/ &model,
    };
}
...
llama_perf_context_data llama_context::perf_get_data() const {
    ...
    data.n_graph_miss = std::max(0, n_graph_miss);
...
void llama_set_mtp_skip_lm_head(llama_context * ctx, bool value) {
    ctx->set_mtp_skip_lm_head(value);
}
...
bool llama_mtp_init_head_shortlist(llama_context * ctx, int32_t n_short) {
    return ctx->init_mtp_head_shortlist(n_short);
}

llama_token llama_mtp_remap_draft_id(const llama_context * ctx, llama_token id) {
    if (!ctx) {
        return id;
    }
    return ctx->remap_mtp_draft_id(id);
}
...
void llama_perf_context_print(const llama_context * ctx) {
    ...
    LLAMA_LOG_INFO("%s:    graphs rebuilt = %10d\n", __func__, data.n_graph_miss);
}
