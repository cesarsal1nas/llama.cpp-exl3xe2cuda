...
static void llama_gen_hadamard(ggml_tensor * tensor) {
    GGML_ASSERT(tensor->type == GGML_TYPE_F32);

    const int n = tensor->ne[0];

    GGML_ASSERT((n & (n - 1)) == 0);
    GGML_ASSERT(tensor->ne[1] == n);
    GGML_ASSERT(tensor->ne[2] == 1);
    GGML_ASSERT(tensor->ne[3] == 1);

    float * data = (float *) tensor->data;
    data[0*n + 0] = 1.0f / sqrtf((float) n);
...
static const std::vector<float> & llama_exl3_h128_data() {
    static const std::vector<float> data = []() {
        ...
        GGML_ASSERT(ctx);
        ggml_tensor * tmp = ggml_new_tensor_2d(ctx, GGML_TYPE_F32, 128, 128);
        tmp->data = v.data();
...
void llm_graph_input_exl3_h128::set_input(const llama_ubatch * ubatch) {
    GGML_UNUSED(ubatch);
    GGML_ASSERT(h128);
    GGML_ASSERT(data);
    ggml_backend_tensor_set(h128, data, 0, ggml_nbytes(h128));
}

bool llm_graph_input_exl3_h128::can_reuse(const llm_graph_params & params) {
    GGML_UNUSED(params);
    return true;
}
...
llm_graph_context::llm_graph_context(const llm_graph_params & params) :
    ...
    res              (params.res),
    model            (params.model),
    mtp_head_shortlist (params.mtp_head_shortlist),
    mtp_head_pad_neg   (params.mtp_head_pad_neg),
    mtp_head_prefix_n  (params.mtp_head_prefix_n),
    ctx0             (res->get_ctx()),
    gf               (res->get_gf()) {
...
ggml_tensor * llm_graph_context::get_exl3_h128() const {
    if (!inp_exl3_h128) {
        auto inp = std::make_unique<llm_graph_input_exl3_h128>(llama_exl3_h128_data().data());
        inp->h128 = ggml_new_tensor_2d(ctx0, GGML_TYPE_F32, 128, 128);
        ggml_set_input(inp->h128);
        ggml_set_name(inp->h128, "exl3_h128");
        inp_exl3_h128 = inp->h128;
        res->add_input(std::move(inp));
    }
    return inp_exl3_h128;
}

static void llama_tensor_set_type(ggml_tensor * t, ggml_type type) {
    t->type = type;
...
static bool llama_exl3_swiglu_f16() {
    static const int on = [] {
        const char * e = getenv("GGML_EXL3_SWIGLU_F16");
        if (!e) {
#ifdef GGML_USE_CUDA
            return 1;
#else
...
static void llama_mul_mat_set_ncols(ggml_tensor * dst, int64_t n_cols) {
    GGML_ASSERT(dst->op == GGML_OP_MUL_MAT);
    GGML_ASSERT(n_cols > 0 && n_cols <= dst->ne[0]);
    dst->ne[0] = n_cols;
    dst->nb[1] = ggml_row_size(dst->type, n_cols);
...
ggml_tensor * llm_graph_context::build_lora_mm(
    ...
    if (ggml_is_exl3(w->type)) {
        ...
        const auto scales = model->get_exl3_scales(w);
        ggml_tensor * suh = scales.first;
        ggml_tensor * svh = scales.second;
        GGML_ASSERT(suh && svh);

        if (n_cols > 0 && n_cols < svh->ne[0]) {
            GGML_ASSERT(n_cols % 128 == 0);
            svh = ggml_view_1d(ctx0, svh, n_cols, 0);
        }

        ggml_tensor * h128 = get_exl3_h128();
        ggml_tensor * x = ggml_mul(ctx0, cur, suh);
        x = llama_mul_mat_hadamard(ctx0, x, h128);
        res = ggml_mul_mat(ctx0, w, x);
        if (n_cols > 0 && n_cols < res->ne[0]) {
            llama_mul_mat_set_ncols(res, n_cols);
        }
        res = llama_mul_mat_hadamard(ctx0, res, h128);
        res = ggml_mul(ctx0, res, svh);
    } else {
        ggml_tensor * w_use = w;
        if (n_cols > 0 && n_cols < w->ne[1]) {
            w_use = ggml_view_2d(ctx0, w, w->ne[0], n_cols, w->nb[1], 0);
        }
        res = ggml_mul_mat(ctx0, w_use, cur);
        if (n_cols > 0 && n_cols < res->ne[0]) {
            llama_mul_mat_set_ncols(res, n_cols);
        }
    }
...
ggml_tensor * llm_graph_context::build_lora_mm_id(
    ...
    if (ggml_is_exl3(w->type)) {
        ...
        ggml_tensor * svh = scales.second;
        GGML_ASSERT(suh && svh);
        GGML_ASSERT(suh->ne[0] == w->ne[0] && suh->ne[1] == w->ne[2]);
        GGML_ASSERT(svh->ne[0] == w->ne[1] && svh->ne[1] == w->ne[2]);
    ...
    switch (type_op) {
        case LLM_FFN_SILU:
            if (gate && type_gate == LLM_FFN_PAR) {
                ...
                if (down && ggml_is_exl3(down->type) && llama_exl3_swiglu_f16()) {
                    llama_tensor_set_type(cur, GGML_TYPE_F16);
                }
