...
int main(int argc, char ** argv) {
    ...
    while (true) {
        ...
        if (draft.empty()) {
            ...
            // IQ/Q: wipe drafted MTP KV; process() rebuilds it. EXL3 keeps it
            // (ExL-style). process() then skips catch-up for those tokens.
            if (ctx_dft && !llama_model_has_exl3(model_tgt)) {
        ...
        // sample before process() so draft-mtp can fuse catch-up + draft-0
        ...
        common_speculative_set_sampled(spec, seq_id, ids);
