...
extern "C" {
    ...
    // Practical default when the user omits -c. Qwen3.5 train ctx is RoPE-only (262144);
    // ExL cache default is 8192.
    LLAMA_API int32_t llama_model_n_ctx_default(const struct llama_model * model);
    ...
    // true if this GGUF loaded any EXL3 weight + suh/svh companions
    LLAMA_API bool    llama_model_has_exl3(const struct llama_model * model);
...
#define LLAMA_STATE_SEQ_FLAGS_ON_DEVICE 2
    ...
    struct llama_perf_context_data {
        ...
        int32_t n_graph_miss; // process_ubatch rebuilt the graph
    };
